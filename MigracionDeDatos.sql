...
FROM
  (
  SELECT DISTINCT FACTURA_NRO, FACTURA_FECHA, COMPRA_PRECIO, SUCURSAL_DIRECCION, FAC_CLIENTE_DNI, FAC_CLIENTE_NOMBRE, FAC_CLIENTE_APELLIDO,PRECIO_FACTURADO, COMPRA_NRO
  FROM gd_esquema.Maestra
  WHERE 
...
--PARA AUTOPARTE
INSERT INTO UNIX.Factura (FACTURA_NRO,FACTURA_FECHA,PRECIO_TOTAL, TIPO_PRODUCTO, CLIENTE_CODIGO)
SELECT FACTURA_NRO, FACTURA_FECHA, SUM(PRECIO_FACTURADO*CANT_FACTURADA), 'AP', Cl.CLIENTE_CODIGO
FROM
  (SELECT DISTINCT FACTURA_NRO, FACTURA_FECHA, FAC_CLIENTE_DNI, FAC_CLIENTE_NOMBRE,
   FAC_CLIENTE_APELLIDO,PRECIO_FACTURADO, CANT_FACTURADA
  FROM  gd_esquema.Maestra 
  WHERE FACTURA_NRO IS NOT NULL and FAC_CLIENTE_DNI IS NOT NULL and  AUTO_PARTE_CODIGO IS NOT NULL) AS T
INNER JOIN UNIX.Cliente Cl ON (Cl.CLIENTE_DNI = T.FAC_CLIENTE_DNI and
Cl.CLIENTE_APELLIDO= T.FAC_CLIENTE_APELLIDO and
Cl.CLIENTE_NOMBRE= T.FAC_CLIENTE_NOMBRE)
GROUP BY FACTURA_NRO, CLIENTE_CODIGO, FACTURA_FECHA
ORDER BY FACTURA_NRO
